...
const SignInScreen = ({ navigation }: any): JSX.Element => {
    ...
    const signIn = async () => {
        ...
        .then( async (response) => {
            if(response?.data?.refresh_token && response.data.access_token) {
                ...
                Alert.alert("Sign In Error", "Invalid response try again");
            }
        }).catch(err => {
    ...
    return(
        <SafeAreaView style={{flex: 1, flexDirection: 'row'}}>
            <View style={[style.circle, {right: -25,top: -55, opacity: .2}]} />
            ...
            <View style={style.container}>
                ...
                <Text style={[GlobleStyle.labelSmall, {alignSelf: 'center', marginBottom: 15}]}>Forgot password? </Text>

                <View style={{flexDirection: 'row', alignSelf: 'center'}}>
                    <Text style={GlobleStyle.labelSmall}>don't have an account? </Text>
                    <TouchableOpacity
                        onPress={() => navigation.navigate('signup_screen')}>
                        <Text style={[GlobleStyle.labelSmall, {color: '#003049'}]}>SIGN UP</Text>
                    </TouchableOpacity>
                </View>
            </View>
